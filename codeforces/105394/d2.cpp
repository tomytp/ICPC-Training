...
struct Peixe {
    ...
    const Peixe operator+ (const Peixe& o) {
        return {sum(n, o.n), c + o.c};
    }
    const Peixe operator- (const Peixe& o) {
        return {sub(n, o.n), c + o.c};
    }
};
...
int main(){
    ...
    vector<Peixe> v_mais(n, {0,0});
    vector<Peixe> v_menos(n, {0,0});
    ...
    forn(i,0,num_q){
        ...
        if(c == '+'){
            ll b, x; cin >> b >> x;
            x--;
            seg_mais.point_add( x, {mult(b, pow_q[x])    , 1});
            seg_menos.point_add(x, {mult(b, pow_inv_q[x]), 1});
            continue;
        }
        if(c == '-'){
            ll b, x; cin >> b >> x;
            x--;
            seg_mais.point_sub( x, {mult(b, pow_q[x])    , 1});
            seg_menos.point_sub(x, {mult(b, pow_inv_q[x]), 1});
            continue;
        }
        ll x; cin >> x;
        ...
        Peixe left = seg_mais.query(0,x);
        Peixe right = seg_mais.query(x+1,n-1);

        resp = sum(mult(pow_inv_q[x], left.n), mult(pow_q[x], right.n));
